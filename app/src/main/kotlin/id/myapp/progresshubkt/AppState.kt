...
class AppState(private val context: Context) {
    ...
    /** Parses a full state JSON blob (as produced by [buildJson]/[exportJson])
     * and applies it to the in-memory state. Returns false (leaving state
     * untouched) if the JSON couldn't be parsed at all. */
    private fun applyJson(raw: String): Boolean {
        return try {
    ...
    private fun save() {
        prefs.edit().putString(storageKey, buildJson().toString()).apply()
    }

    /** Serializes the full app state (settings + progress) as pretty JSON,
     * for the "Export data" action in Settings. */
    fun exportJson(): String = buildJson().toString(2)

    /** Restores state from a JSON blob previously produced by [exportJson],
     * e.g. via the "Import data" action in Settings. Persists on success. */
    fun importData(raw: String): Boolean {
        val ok = applyJson(raw)
        if (ok) {
            week = week.coerceIn(1, settings.totalWeeks)
            save()
        }
    ...
    /** Clears all logged progress (completed sessions, distances, weight
     * history) and restarts at week 1 — but keeps the program settings
     * (start/goal weight, weeks, accent color) untouched. Used by the
     * "Reset Program" danger-zone action in Settings. */
    fun resetProgress() {
        completed = emptyMap()
        actualKm = emptyMap()
        weights = mapOf("0" to settings.startWeight)
...
private fun settingsToJson(s: ProgramSettings): JSONObject {
    ...
    j.put("accentColorArgb", s.accentColorArgb)
    return j
}
...
private fun settingsFromJson(j: JSONObject): ProgramSettings {
    ...
    return ProgramSettings.normalize(
        ProgramSettings(
            ...
            restDayKeys = rest,
            accentColorArgb = j.optLong("accentColorArgb", d.accentColorArgb)
        )
    )
}
