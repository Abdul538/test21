...
data class ProgramSettings(
    ...
    val accentColorArgb: Long = 0xFF5FB3A3
) {
...
class Phase(val name: String, val colorHex: Long, val description: String)
...
fun phaseFor(week: Int, totalWeeks: Int): Phase {
    ...
    return when {
        week <= b.b1 -> Phase(
            "Rebuild Base",
            0xFF5FB3A3,
            "Membangun kebiasaan dan daya tahan dasar dengan volume ringan-sedang, sebelum beban latihan mulai naik."
        )
        week <= b.b2 -> Phase(
            "Ramp Up",
            0xFFE0A94E,
            "Volume latihan naik bertahap tiap minggu untuk membangun stamina menuju target penuh."
        )
        week <= b.b3 -> Phase(
            "Target Volume",
            0xFFD16B5C,
            "Volume latihan berada di titik tertinggi, dengan minggu deload setiap 4 minggu untuk pemulihan."
        )
        else -> Phase(
            "Maintain & Deload",
            0xFF7A8BA6,
            "Volume diturunkan untuk menjaga hasil dan memulihkan tubuh menjelang akhir program."
        )
    }
}
